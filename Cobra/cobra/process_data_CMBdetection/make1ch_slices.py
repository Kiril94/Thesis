...
main_path = "/home/lkw918/cobra/data/Synth_CMB_sliced_new"
folders = next(os.walk(main_path))[1]
#filter test folder
folders_set = list(filter(lambda x: x!="test",folders))

for folder_set in folders_set:

    for folder_view in next(os.walk(f"{main_path}/{folder_set}"))[1]:

        print(folder_set,folder_view)
        files = next(os.walk(f"{main_path}/{folder_set}/{folder_view}/images/"))[2]

        for file in files:
            img,meta = load_nifti_img(f"{main_path}/{folder_set}/{folder_view}/images/{file}")
            nib.save(nib.Nifti1Image(img[:,:,1],meta['affine']),f"{main_path}/{folder_set}/{folder_view}/images_1ch/{file}")
