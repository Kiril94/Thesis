...
#     gibbsRinging, motionGhosting, multiplicativeNoise
import numpy as np
...
class contrastStretch(AbstractTransform):
    ...
    def __init__(self, data_key="data", p_per_sample=1,
        ...
        if not self.p_per_sample == 1 or not self.p_per_sample == 0:
...
class gibbsRinging(AbstractTransform):
    ...
    def __gibbsRinging__(self, imageVolume, numSample, dim):
        ...
        if dim == 0:
            ...
            imageVolume[:, :, int(np.ceil(h / 2) + np.ceil(numSample / 2)) : w] = 0
            ...
            imageVolume[:, :, int(np.ceil(d / 2) + np.ceil(numSample / 2)) : w] = 0
