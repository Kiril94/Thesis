...
class NLLH:
    ...
    def __init__(self, f, data):
        ...
        # Function signature for the minimizer
...
class NLLH_scan:
    ...
    def __call__(self, *par):  # par are a variable number of model parameters
        ...
        for i in range(len(par)):
            # separate into float and array to create a meshgrid
            if not(type(par[i]) == float or type(par[i]) == int):
        ...
        for i in range(len(Par_grid)):
            # expand dimensions to make function call in parallel
            Par_grid_expanded[i] = np.expand_dims(Par_grid[i], len(index))
            # this allows to broadcast data and params
            Par_grid_expanded[i] = np.repeat(
                Par_grid_expanded[i], len(self.data), axis=len(index))
        ...
        # sum over the axis that represents the data points(last axis)
        llh = -np.sum(logf, axis=len(index))
        llh[~pos_mask] = -np.inf
        return Par_grid, llh
